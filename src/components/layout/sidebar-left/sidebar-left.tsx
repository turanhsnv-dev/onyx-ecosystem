"use client"; // Hook işlətmək üçün vacibdir

import { usePathname } from "next/navigation"; // URL-i oxumaq üçün
...
export const SidebarLeft = () => {
  const pathname = usePathname(); // Cari URL-i alırıq (məs: "/store")
      ...
      <nav className="flex flex-col gap-1 flex-1">
        {MAIN_NAV.map((item) => (
          <NavItem
            key={item.title}
            {...item}
            // BURADA MƏNTİQ: Əgər pathname bu item-in href-idirsə, isActive true olur
